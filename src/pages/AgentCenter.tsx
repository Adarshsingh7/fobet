...
import { 
  Zap, Database, Brain, Loader2, Sparkles, 
  Target, TrendingUp, Lightbulb, MessageSquare, 
  Hash, Key, Activity, ShieldAlert, Cpu
} from 'lucide-react';
...
const OMEGA_X_CONFIG = {
  ...
  "psychology_stack": [
    "Zeigarnik Effect", "Pattern Recognition Betrayal", "Loss Aversion", 
    "Identity Threat", "Variable Reward", "Social Proof Anxiety", 
    "Tribal Signaling", "Contrast Effect"
  ],
  "output_schema": {
    "hooks": "string[]",
...
export default function AgentCenter({ agentStates, runAgent, runAll, reelInput }: any) {
  ...
  const { data: analysis, isLoading: analysisLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`http://localhost:5000/api/runs/${selectedRunId}/analysis`);
      return res.json();
    },
  ...
  const omegaMutation = useMutation({
    mutationFn: async (runId: string) => {
      const reelsRes = await fetch(`http://localhost:5000/api/runs/${runId}/reels`);
      ...
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${GEMINI_API_KEY}`, {
        ...
        body: JSON.stringify({
          contents: [{ parts: [{ text: `${JSON.stringify(OMEGA_X_CONFIG.system_prompt)}\n\nINPUT DATA:\n${JSON.stringify(aiInput)}` }] }],
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <header className="bg-card border border-border rounded-2xl p-6 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold tracking-tight text-foreground leading-none">Intelligence Command</h2>
            <p className="text-xs text-muted-foreground mt-1 font-medium uppercase tracking-widest opacity-60">VIRALOS OMEGA X v3.1</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
          <div className="relative w-full sm:w-64">
            ...
            >
              ...
              {runs.map((run: any) => (
                <option key={run._id} value={run._id}>{run.name || 'Untitled Run'}</option>
              ))}
            </select>
          </div>
          ...
          >
            {omegaMutation.isPending ? <Loader2 className="animate-spin mr-2" size={14} /> : <Zap size={14} className="mr-2 fill-current" />}
      ...
      {/* AGENT TABS */}
      <Tabs defaultValue="omega" className="space-y-6">
        <div className="flex items-center justify-between border-b border-border pb-1">
          <TabsList className="bg-transparent h-auto p-0 gap-1 flex-wrap justify-start">
            ...
            >
              <Sparkles size={12} className="mr-2" /> OMEGA X Output
            </TabsTrigger>
        ...
        {/* OMEGA X CONTENT */}
        <TabsContent value="omega" className="mt-0 focus-visible:outline-none">
          {analysisLoading ? (
            <div className="h-[400px] flex flex-col items-center justify-center gap-4 text-muted-foreground/30">
              <Loader2 className="animate-spin" size={32} />
              <p className="text-xs font-bold uppercase tracking-[0.2em]">Accessing Intelligence Bank...</p>
            </div>
          ) : !analysis ? (
            <div className="h-[400px] flex flex-col items-center justify-center gap-4 text-center border-2 border-dashed border-border rounded-3xl bg-muted/5">
              <Brain size={48} className="text-muted-foreground/10" />
              <div className="space-y-1">
                <p className="text-sm font-bold text-muted-foreground">Neural Pipeline Ready</p>
                <p className="text-xs text-muted-foreground/60">Select a run and execute the parallel pipeline to generate OMEGA X insights.</p>
              </div>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in slide-in-from-bottom-4 duration-700">
              <AnalysisSection title="Viral Hooks" icon={<Zap className="text-amber-500" />} items={analysis.hooks} psychology="Variable Reward" />
              <AnalysisSection title="Retention Matrix" icon={<Activity className="text-blue-500" />} items={analysis.retensionAnal} psychology="Zeigarnik Effect" />
              <AnalysisSection title="Topic Gap Map" icon={<Target className="text-emerald-500" />} items={analysis.topicGapMap} psychology="Novelty Seeking" />
              <AnalysisSection title="Pattern Recog" icon={<Brain className="text-purple-500" />} items={analysis.patternRecogonition} psychology="Pattern Betrayal" />
              <AnalysisSection title="Strategy Lab" icon={<TrendingUp className="text-primary" />} items={analysis.contentStrategy} psychology="Identity Threat" />
              <AnalysisSection title="Keyword Signal" icon={<Key className="text-cyan-500" />} items={analysis.importantKeywords} psychology="Tribal Signaling" />
                ...
                <Card className="bg-muted/10 border-border rounded-2xl overflow-hidden">
                  <CardHeader className="p-4 border-b border-border bg-background/50">
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="text-[10px] font-black uppercase tracking-widest">Leverage Hashtags</h4>
                    </div>
                  </CardHeader>
                  <CardContent className="p-4 flex flex-wrap gap-2">
                    {analysis.importantHastags?.map((tag: string, i: number) => (
                      <Badge key={i} variant="outline" className="px-3 py-1 text-[11px] font-medium bg-background border-border hover:border-primary transition-colors cursor-default">
                        {tag}
                      </Badge>
                    ))}
                  </CardContent>
        ...
        {AGENT_DEFS.map(agent => (
          <TabsContent key={agent.id} value={agent.id} className="mt-0 focus-visible:outline-none">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <div className="lg:col-span-4 space-y-6">
                <Card className="bg-card border-border rounded-2xl overflow-hidden shadow-sm">
                  <div className={`h-1.5 w-full ${agent.color.replace('text-', 'bg-')}`} />
                  <CardHeader className="p-6">
                    <div className="flex items-center gap-3">
                      <div className={`p-2.5 rounded-xl bg-background border border-border ${agent.color}`}>
                        <agent.icon size={20} />
                      </div>
                      <CardTitle className="text-sm font-bold uppercase tracking-tight">{agent.name}</CardTitle>
                    </div>
                    <CardDescription className="text-xs mt-2 leading-relaxed">{agent.desc}</CardDescription>
                  </CardHeader>
                  <CardContent className="p-6 pt-0 space-y-4">
                    <Button 
                      className="w-full font-bold uppercase tracking-widest h-11 rounded-xl shadow-lg shadow-primary/10"
                      disabled={!reelInput || agentStates[agent.id]?.status === 'running'}
                      onClick={() => runAgent(agent.id)}
                    >
                      {agentStates[agent.id]?.status === 'running' ? <Loader2 className="animate-spin mr-2" size={16} /> : <Zap size={16} className="mr-2" />}
                ...
                <Card className="bg-background border-border rounded-2xl p-5 space-y-3">
                  <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">
                    <ShieldAlert size={12} className="text-amber-500" /> Security Status
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                    <span className="text-[10px] font-bold uppercase tracking-tight">Anti-Hallucination Active</span>
                  </div>
                </Card>
              </div>
                ...
                <Card className="bg-card border-border rounded-2xl h-full min-h-[400px] flex flex-col overflow-hidden shadow-xl">
                  <div className="p-4 border-b border-border bg-muted/20 flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-widest">Neural Stream</span>
                    </div>
                    <Badge variant="outline" className="text-[9px] font-black uppercase border-none bg-primary/5 text-primary">
                  ...
                  <ScrollArea className="flex-1 p-6 bg-background/50">
                    <pre className="font-mono text-[11px] leading-relaxed whitespace-pre-wrap text-foreground/80">
                      {agentStates[agent.id]?.stream || agentStates[agent.id]?.data ? 
                        JSON.stringify(agentStates[agent.id]?.data || agentStates[agent.id]?.stream, null, 2) : 
                    ...
                    {agentStates[agent.id]?.status === 'running' && (
                      <div className="mt-4 flex items-center gap-2 text-primary font-bold text-[9px] uppercase tracking-widest animate-pulse">
                        <Loader2 className="animate-spin" size={12} /> Receiving Uplink...
                      </div>
                    )}
                  </ScrollArea>
                </Card>
...
function AnalysisSection({ title, icon, items, psychology }: { title: string, icon: React.ReactNode, items: string[], psychology: string }) {
  return (
    <Card className="bg-card border-border rounded-2xl overflow-hidden hover:border-primary/20 transition-all duration-300 group">
      <CardHeader className="p-4 pb-2 border-b border-border/40 bg-muted/10">
        <div className="flex justify-between items-center">
          ...
          <Badge variant="secondary" className="bg-background border-border text-[8px] font-black uppercase tracking-tighter h-5">
            {psychology}
          </Badge>
        </div>
      </CardHeader>
      <CardContent className="p-4 space-y-3">
        {items?.slice(0, 5).map((item, i) => (
          <div key={i} className="flex gap-3 text-[11px] leading-relaxed font-medium text-muted-foreground group-hover:text-foreground transition-colors">
            <span className="text-primary/40 shrink-0 font-black">0{i+1}</span>
