...
import { 
  ...
  Terminal as AgentIcon, LineChart, Cpu, 
  ChevronRight, Power, Download, Activity,
  Circle,
} from 'lucide-react';
...
import { AGENT_DEFS, type AgentState, type ScrapeResult } from './lib/constants';
...
export default function App() {
  ...
  const scraperMutation = useMutation({
    mutationFn: async ({ urls, usernames, runName }: { urls: string[], usernames: string[], runName?: string }) => {
      ...
      try {
        const lRunRes = await fetch('http://localhost:5000/api/runs', {
          ...
          body: JSON.stringify({ status: 'pending', name: runName || undefined })
        });
        if (lRunRes.ok) {
      ...
      // 2. Start Apify
      const runRes = await fetch(`https://api.apify.com/v2/acts/${APIFY_ACTOR_ID}/runs?token=${APIFY_API_KEY}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ directUrls: urls, username: usernames.length > 0 ? usernames : urls, resultsLimit: 100 })
      });
      const runData = await runRes.json();
      if (!runRes.ok) throw new Error(runData.error?.message || 'Scraper start failed');
      ...
      while (status === 'RUNNING' || status === 'READY') {
        ...
        const statusRes = await fetch(`https://api.apify.com/v2/acts/${APIFY_ACTOR_ID}/runs/${apifyRunId}?token=${APIFY_API_KEY}`);
      ...
      const datasetRes = await fetch(`https://api.apify.com/v2/datasets/${datasetId}/items?token=${APIFY_API_KEY}`);
    ...
    onSuccess: (data) => {
      const processed: ScrapeResult[] = data.items.map((item: any) => ({
        ...
        transcript: item.transcript || item.videoTranscript || item.description || '',
        timestamp: new Date().toISOString()
      }));
      
      setScrapedData(processed);
      const newContext = processed.map(d => `[SCRAPED_NODE]\nURL: ${d.url}\nCAPTION: ${d.caption}\n${d.transcript ? `TRANSCRIPT: ${d.transcript}\n` : ''}---`).join('\n\n');
  ...
  const startScraper = (urls: string[], usernames: string[], runName?: string) => {
    setIsScraping(true);
    setScrapeError(null);
  ...
  const runAgent = async (agentId: string) => {
    ...
    setAgentStates(prev => ({ ...prev, [agentId]: { ...prev[agentId], status: 'running', stream: '' } }));
    ...
    try {
          ...
          for (const line of lines) {
            if (line.startsWith('data: ')) {
              try {
                ...
                fullText += text;
                setAgentStates(prev => ({ ...prev, [agentId]: { ...prev[agentId], stream: fullText } }));
              } catch (e) {}
            }
          }
        }
      ...
      setAgentStates(prev => ({ ...prev, [agentId]: { status: 'done', data: result, stream: '' } }));
    } catch (e) {
      setAgentStates(prev => ({ ...prev, [agentId]: { status: 'error', data: null, stream: 'Agent Failed to synthesize context.' } }));
    }
  };
        ...
        <aside className="w-64 border-r border-border bg-card/30 flex flex-col z-20 backdrop-blur-md">
          <div className="h-14 flex items-center px-5 border-b border-border/40 gap-3">
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-primary shadow-sm">
              <Terminal className="text-primary-foreground stroke-[2.5px]" size={14} />
            </div>
            <div className="flex flex-col">
              <span className="text-[14px] font-bold tracking-tight text-foreground leading-none">ReelOS</span>
              <span className="text-[10px] font-medium text-muted-foreground/50 uppercase tracking-widest mt-0.5">v2.0 Neural</span>
            </div>
          </div>
          ...
          <nav className="flex-1 px-3 py-6 space-y-1">
            {sidebarItems.map((item) => (
              ...
              >
                <item.icon size={16} className={`${activeTab === item.id ? 'text-primary' : 'opacity-60 group-hover:opacity-100 transition-opacity'}`} />
                {item.label}
                {item.id === 'agents' && doneCount > 0 && (
                  <Badge variant="secondary" className="ml-auto h-5 px-1.5 text-[10px] bg-primary/10 text-primary border-none rounded-full">
                    {doneCount}
                  </Badge>
                )}
              </button>
          ...
          <div className="p-4 m-3 rounded-xl bg-card border border-border/60 space-y-4">
            <div className="space-y-2">
              <div className="flex justify-between items-center text-[11px] font-medium">
                ...
                <span className="text-foreground tabular-nums">{Math.round((doneCount / 6) * 100)}%</span>
              </div>
              <Progress value={(doneCount / 6) * 100} className="h-1 bg-muted rounded-full" />
            </div>
        ...
        <div className="flex-1 flex flex-col relative overflow-hidden">
          ...
          <header className="h-14 border-b border-border bg-background/50 backdrop-blur-md flex items-center justify-between px-6 z-10">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-[12px] font-semibold text-foreground capitalize tracking-tight">{activeTab}</span>
            </div>
            
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1.5 mr-2">
                <Badge variant="outline" className="text-[11px] font-medium h-7 px-2.5 border-border rounded-lg bg-card/50">
          ...
          <main className="flex-1 overflow-y-auto scroll-smooth">
            <div className="max-w-5xl mx-auto p-8 lg:p-12 min-h-full">
              <div className="transition-all duration-300">
                <Routes>
                  <Route path="/" element={<Dashboard reelCount={reelCount} doneCount={doneCount} agentStates={agentStates} />} />
                  <Route path="/input" element={<ReelInput 
                    reelInput={reelInput} 
                  ...
                  />} />
                  <Route path="/agents" element={<AgentCenter agentStates={agentStates} runAgent={runAgent} runAll={runAll} reelInput={reelInput} />} />
                  <Route path="/insights" element={<InsightsBoard agentStates={agentStates} />} />
                  <Route path="/recommendations" element={<StrategyLab agentStates={agentStates} />} />
                  <Route path="/export" element={<Archive reelInput={reelInput} agentStates={agentStates} />} />
                </Routes>
              </div>
            </div>
          </main>
